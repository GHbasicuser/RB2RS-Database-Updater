...
'db-update.vbs v°1.05 (10/05/2022) par GHbasicuser (aka PhiliWeb)'
...
RadioSure = 0 'Mettre 1 pour démarrer RadioSure en fin de script, sinon 0'
'-----------------------------------------------------------------------------'
Dim oMessageBox
...
If Not FS.FileExists("RadioSure.exe") Then 
       oMessageBox.Popup "Problem: This VBS script doesn't seem to be running in the RadioSure folder. (message generated by db-update.vbs)"
       Set oMessageBox = Nothing
       wscript.Quit
End If
'Fonction pour lancer (ou pas) RadioSure à la fin du script'
...
Set Fichier = FS.GetFile("Stations\Latest_RB2RS.zip")   
    If DateDiff("h", Fichier.DateLastModified, Now) < 12 Then 
       If RadioSure = 1 Then Start_RadioSure() 
       wscript.Quit
    End  If 
    If DateDiff("d", Fichier.DateLastModified, Now) > 30 Then 
       oMessageBox.Popup "RadioSure - The last successful update is more than 30 days old. (message generated by db-update.vbs)"
       Set oMessageBox = Nothing
    End If
Set Fichier = Nothing
End If    
...
Else 
     Set xHttp = Nothing
     oMessageBox.Popup "RadioSure - Didn't get a response from Stations list Update server. (message generated by db-update.vbs)", 10
     Set oMessageBox = Nothing
     If RadioSure = 1 Then Start_RadioSure()
     wscript.Quit
End If
...
If Fichier.Size < 1000000 Then 
   Set Fichier = Nothing
   oMessageBox.Popup "RadioSure - The downloaded ZIP file seems too small. Update cancelled. (message generated by db-update.vbs)", 10
   Set oMessageBox = Nothing
   If RadioSure = 1 Then Start_RadioSure()
   wscript.Quit
End If
...
oMessageBox.Popup "RadioSure - The Radio Stations database has been updated.", 5
Set oMessageBox = Nothing
If RadioSure = 1 Then Start_RadioSure()
